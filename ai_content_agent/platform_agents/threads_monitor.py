...
def _check_credentials() -> bool:
    if not META_ACCESS_TOKEN or not THREADS_USER_ID:
        logger.info("META_ACCESS_TOKEN/THREADS_USER_ID not set — skipping Threads")
        return False
    return True
...
def post_reply(thread_id: str, text: str) -> Optional[str]:
    """Reply to a Threads post via the API. Returns the new post URL or None."""
    ...
    pub_resp = requests.post(f"{THREADS_BASE}/{THREADS_USER_ID}/threads_publish", data={
    ...
    return f"https://www.threads.net/t/{pub_resp.json().get('id', '')}"
